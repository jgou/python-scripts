...
- Supports a `--services` flag to limit which AWS services are scanned/deleted, comma-separated (`clientvpn`, `cloudfront`, `ec2`, `eip`, `elasticache`, `elb`, `internetgateway`, `lambda`, `natgateway`, `networkfirewall`, `outboundresolver`, `rds`, `route53`, `s3`, `transitgateway`, `vpcendpoint`, `vpn`)
...
```bash
python3 aws-cleanup-helper --profile my-aws-profile --services ec2,elasticache,elb,rds,route53,s3,natgateway,eip,vpcendpoint,vpn,transitgateway,clientvpn,networkfirewall,cloudfront,lambda,internetgateway,outboundresolver --dry-run
```
