...
> [!WARNING]
> Deleting a CloudTrail trail removes account audit history and is not reversible. If the trail has S3 data events enabled account-wide, every list/delete API call this tool (or anything else) makes gets logged and billed as a data event — including calls against the trail's own log-destination bucket, which can quietly become a significant cost driver during heavy cleanup activity. `cloudtrail` is deleted first, before any other service, specifically to stop that.
...
- Scans and reports all S3 buckets in the account, their region, and whether they contain objects (including noncurrent versions and delete markers in versioned buckets)
- Deletes every object version and delete marker in each bucket (not just current versions, so versioned buckets are actually emptied instead of just growing delete markers), then the bucket itself
- Supports a `--skip-s3-bucket` flag to exclude specific bucket names from S3 cleanup, comma-separated (they're still reported by the scan, just skipped on delete)
...
- Scans and reports all CloudWatch Log Groups across regions and their stored bytes; disables deletion protection where enabled, then deletes each one
- Scans and reports all CloudWatch Alarms (metric and composite) across regions, then deletes them
- Scans and reports all CloudWatch Dashboards (account-wide, not regional), then deletes them
- Scans and reports all CloudTrail trails (account-wide, including ones homed in other regions) and whether each is an AWS Organizations trail; stops logging and deletes each one (Organizations trails are skipped, since they can only be deleted from the management account) — run before every other service
- Supports a `--regions` flag to limit which AWS regions are scanned (all services except S3, Route 53, CloudFront, CloudWatch Dashboards, and CloudTrail, which are always listed account-wide/global), comma-separated. Default is all regions.
- Supports a `--services` flag to limit which AWS services are scanned/deleted, comma-separated (`ami`, `clientvpn`, `cloudfront`, `cloudtrail`, `cloudwatchalarms`, `cloudwatchdashboards`, `cloudwatchlogs`, `ebssnapshot`, `ebsvolume`, `ec2`, `eip`, `elasticache`, `elb`, `internetgateway`, `lambda`, `natgateway`, `networkfirewall`, `outboundresolver`, `rds`, `route53`, `s3`, `transitgateway`, `vpcendpoint`, `vpn`)
